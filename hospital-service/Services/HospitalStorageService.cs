...
namespace HospitalService.Services
{
    public class HospitalStorageService : IHospitalStorageService
    {
        ...
        public async Task<Contracts.V2.Company> GetCompanyAsync(string guid, CancellationToken cancellationToken)
        {
            ...
            if (string.IsNullOrWhiteSpace(data)) throw new Exception("Unable to get Company");
        ...
        {
            var data = await _cache.GetStringAsync(guid, cancellationToken);
            if (string.IsNullOrWhiteSpace(data)) throw new Exception("Unable to get Hospital");
        ...
        {
            var data = await _cache.GetStringAsync(guid, cancellationToken);
            if (string.IsNullOrWhiteSpace(data)) throw new Exception("Unable to get Department");
        ...
        {
            var data = await _cache.GetStringAsync(guid, cancellationToken);
            if (string.IsNullOrWhiteSpace(data)) throw new Exception("Unable to get Doctor");
